...
Your one and only job:
  ...
  3. Wait for kb_record_remote to respond with a real case_id.
     Return that exact case_id in your structured output.

Do NOT invent or fabricate a case_id. If you cannot get one from
kb_record_remote, say so — never make up a UUID.
""".strip()
